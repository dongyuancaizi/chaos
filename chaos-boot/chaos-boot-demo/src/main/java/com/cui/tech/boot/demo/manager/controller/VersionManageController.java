...
import com.cui.tech.chaos.model.PageResult;
...
public class VersionManageController extends BaseController {
    ...
    public Result<MU> add(@RequestBody @Validated VersionData data, BindingResult bindingResult) throws Exception {
    ...
    public Result<Boolean> update(
            @ApiParam(value = "更新版本管理") @RequestBody @Validated VersionUpdateData data, BindingResult bindingResult) throws Exception {
        return getResult(bindingResult, iVersionManageService.updateModelByMU(data));
    }

    @PostMapping("/one")
    @ApiOperation(value = "查询版本管理", notes = "", httpMethod = "POST")
    public Result<Version> one(
            @RequestBody MU data) throws Exception {
    ...
    public Result<List<Version>> list(
